...
class mgs_trx_1(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, bb_gain=1, gs_name='MGS', ip='0.0.0.0', iq_file='./rocksat_125kbd_500ksps_date_comment.dat', meta_rate=.1, port='52001', record_iq=0, record_rfo=0, record_snr=0, rfo_file='./rocksat_rfo_date_comment.meta', snr_file='./rocksat_snr_date_comment.meta', tx_correct=0, tx_freq=1265e6, tx_offset=250e3):
        ...
        self.iq_fp = iq_fp = "/home/zleffke/captures/rocksat/mgs/{:s}".format(iq_fn)
    ...
    def set_iq_fn(self, iq_fn):
        self.iq_fn = iq_fn
        self.set_iq_fp("/home/zleffke/captures/rocksat/mgs/{:s}".format(self.iq_fn))
...
def argument_parser():
    ...
    parser.add_option(
        "", "--gs-name", dest="gs_name", type="string", default='MGS',
...
def main(top_block_cls=mgs_trx_1, options=None):
    ...
    tb = top_block_cls(bb_gain=options.bb_gain, gs_name=options.gs_name, ip=options.ip, iq_file=options.iq_file, meta_rate=options.meta_rate, port=options.port, record_iq=options.record_iq, record_rfo=options.record_rfo, record_snr=options.record_snr, rfo_file=options.rfo_file, snr_file=options.snr_file, tx_correct=options.tx_correct, tx_freq=options.tx_freq, tx_offset=options.tx_offset)
    tb.start()
    tb.show()
