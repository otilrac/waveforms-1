...
# Title: Ais Rx
# Generated: Fri May 26 18:01:39 2017
##################################################
...
class ais_rx(gr.top_block):
    ...
    def __init__(self, rx_gain=38, addr='0.0.0.0', port='52001'):
        ...
        self.blocks_socket_pdu_0 = blocks.socket_pdu("UDP_CLIENT", addr, port, 10000, False)
...
def main(top_block_cls=ais_rx, options=None):
    ...
    tb = top_block_cls(rx_gain=options.rx_gain, addr=options.addr, port=options.port)
    tb.start()
    try:
